...
const Item = props => {
    ...
    const handleCheckboxChange = () => dispatch(toggleTodo(props.item.id))
    ...
    const editor = isEditing ? <input onChange={handleEditorChange} onKeyUp={event => event.keyCode === 13 ? handleClick() : null} autoFocus type="text" className="form-control" />
                             : <span className="d-flex align-items-baseline"><input onChange={handleCheckboxChange} type="checkbox" className="mr-2" checked={props.item.completed} /><h5 onClick={handleTitleClick} style={props.item.completed ? done : { cursor: 'pointer' }}>{firstUpperCase(props.item.title)}</h5></span>
